...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward_x(
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if rank_layer in self.layer_list:
                if hidden_states.shape[1] != 1:  # prefill stage or training
                    ...
                    stage = self.layer_list.index(rank_layer)  # determine current stage
                    cur_visual_length = [min(int(self.image_token_list[stage]), cur_image_token) for cur_image_token in
                                         self.image_tokens]  # [576]
                    next_visual_length = [min(int(self.image_token_list[stage + 1]), cur_image_token) for
                                          cur_image_token in self.image_tokens]  # [288]
        ...
        if not return_dict:
            return tuple(
                v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns] if v is not None), labels
        return BaseModelOutputWithPast(
            last_hidden_state=hidden_states,
            past_key_values=next_cache,
            hidden_states=all_hidden_states,
    ...
    def layer_prune(
            ...
            position_ids, attention_mask, labels, selected_indices, sum_visual_attention, indices_attention
    ):
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            # If no image input, set 0
            image_tokens = [min(int(self.image_token_list[cur_num]), cur_image_token) for cur_image_token in
                            self.image_tokens]  # [576]
            keep_length = [min(int(self.image_token_list[cur_num + 1]), cur_image_token) for cur_image_token in
                           self.image_tokens]  # [288]
            ...
            if attention_mask is None:
                attention_mask = torch.ones((batch_size, features.shape[1]), dtype=torch.bool,
                                            device=features.device)  # (1, 362)
            else:
            ...
            for i in range(batch_size):
                ...
                text_index = image_index + image_tokens[i]  # 35 + 576
                ...
                errors_indices = torch.topk(errors, k=keep_length[i], dim=0)[1]

                image_embeds[errors_indices[-k:]] = seed_features
                image_embeds = token_merging(image_embeds, errors_indices, k)

                image_embeds = image_embeds.half()
                seed_features = seed_features.half()
                errors_indices = torch.sort(errors_indices, dim=0)[0]

                new_input_embeds = torch.cat(
                    [features[i][:image_index, :], image_embeds[errors_indices], features[i][text_index:, :]],
                    dim=0)  # (362, 4096)
                new_labels = torch.cat([labels[i][:image_index], labels[i][errors_indices], labels[i][text_index:]],
                                       dim=0)
            ...
            for i, (cur_new_embed, cur_new_labels) in enumerate(zip(new_input_embeds, new_labels)):
                ...
                cur_new_embed = torch.cat([cur_new_embed,
                                           torch.zeros((dif, cur_new_embed.shape[1]), dtype=cur_new_embed.dtype,
                                                       device=cur_new_embed.device)], dim=0)
                cur_new_labels = torch.cat([cur_new_labels, torch.full((dif,), IGNORE_INDEX, dtype=cur_new_labels.dtype,
                                                                       device=cur_new_labels.device)], dim=0)
                cur_attention_mask = new_attention_mask[i]
                cur_attention_mask = torch.cat([cur_attention_mask,
                                                torch.full((dif,), False, dtype=cur_attention_mask.dtype,
                                                           device=cur_attention_mask.device)], dim=0)
